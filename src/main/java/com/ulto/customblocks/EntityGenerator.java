                ...
                if (aiTasks.get(i).isJsonPrimitive()) {
                    if (aiTasks.get(i).getAsJsonPrimitive().isString()) {
                        switch (aiTasks.get(i).getAsString()) {
                            case "break_door" -> goalSelector.add(i, new BreakDoorGoal(pathAwareEntity, (difficulty) -> difficulty.getId() > 0));
                            case "use_door" -> goalSelector.add(i, new DoorInteractGoal(pathAwareEntity) {});
                            case "wander" -> goalSelector.add(i, new WanderAroundGoal(pathAwareEntity, 1));
                            case "look_around" -> goalSelector.add(i, new LookAroundGoal(pathAwareEntity));
                            case "float" -> goalSelector.add(i, new SwimGoal(pathAwareEntity));
                        }
                    }
                            ...
                            case "attack" -> {
                                ...
                                if (aiTask.has("targets")) {
                                    for (String target : JsonUtils.jsonArrayToStringList(aiTask.getAsJsonArray("targets"))) {
                                        Class<? extends LivingEntity> targetEntity = (Class<? extends LivingEntity>) Registry.ENTITY_TYPE.get(new Identifier(target)).getBaseClass();
                                        targetSelector.add(i, new ActiveTargetGoal<>(pathAwareEntity, targetEntity, true));
                                    }
                                }
                            }
                            case "look_at_entity" -> {
                                if (aiTask.has("target")) {
                                    Class<? extends LivingEntity> targetEntity = (Class<? extends LivingEntity>) Registry.ENTITY_TYPE.get(new Identifier(aiTask.get("target").getAsString())).getBaseClass();
