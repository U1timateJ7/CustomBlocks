    ...
    public static boolean add(JsonObject tree) {
        if (tree.has("namespace") && tree.has("id") && tree.has("trunk_block") && tree.has("leaves_block")) {
            ...
            TreeFeatureConfig.Builder builder = new TreeFeatureConfig.Builder(
                    ...
                    new SimpleBlockStateProvider((hasSapling ? Registry.BLOCK.get(new Identifier(id.getNamespace(), getSaplingId(id.getPath()))) : Blocks.AIR).getDefaultState()), // Sapling provider; used to determine what blocks the tree can generate on
            ...
            ConfiguredFeature<?, ?> TREE = Feature.TREE.configure(builder.build()).decorate(Decorator.HEIGHTMAP.configure(new HeightmapDecoratorConfig(Heightmap.Type.MOTION_BLOCKING))).spreadHorizontally().applyChance(3);
            trees.put(id, Feature.TREE.configure(builder.build()));
            RegistryKey<ConfiguredFeature<?, ?>> customTree = RegistryKey.of(Registry.CONFIGURED_FEATURE_KEY, id);
            ...
            if (bees) {
                ...
                builder.decorators(decorators);
                ConfiguredFeature<?, ?> TREE_BEES = Feature.TREE.configure(builder.build()).decorate(Decorator.HEIGHTMAP.configure(new HeightmapDecoratorConfig(Heightmap.Type.MOTION_BLOCKING))).spreadHorizontally().applyChance(3);
                trees.put(new Identifier(id.getNamespace(), id.getPath() + "_bees"), Feature.TREE.configure(builder.build()));
                RegistryKey<ConfiguredFeature<?, ?>> customTreeBees = RegistryKey.of(Registry.CONFIGURED_FEATURE_KEY, new Identifier(id.getNamespace(), id.getPath() + "_bees"));
